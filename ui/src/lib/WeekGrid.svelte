...
<script lang="ts">
  ...
  function trackEventHover(day: DayColumn, e: PointerEvent) {
    ...
    const placed = dayLayouts.get(day.start_ms)?.placed ?? day.placed;
    const underTime = placed.filter(p => y >= p.top + 1 / r.height && y < p.top + p.height - 1 / r.height);
  ...
  const hoverContext = $derived.by(() => {
    ...
    const day = renderedDays.find(d => d.start_ms === hover.day);
    const placed = day && (dayLayouts.get(day.start_ms)?.placed ?? day.placed);
    const active = placed?.find(p => p.idx === hover.idx);
    ...
    const peers = placed!.filter(p => containsPlacement(active, p));
